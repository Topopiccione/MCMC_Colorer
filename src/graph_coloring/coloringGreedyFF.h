...
namespace ColoringGreedyFF_k{
    template<typename nodeW, typename edgeW>
    __global__ void tentative_coloring(const uint32_t numNodes, const uint32_t* input_coloring, uint32_t* output_coloring,
			const node_sz * const cumulDegs, const node * const neighs, uint32_t* forbidden_colors, const uint32_t maxColors);

    template<typename nodeW, typename edgeW>
    __global__ void conflict_detection(const uint32_t numNodes, const uint32_t* input_coloring, uint32_t* output_coloring,
			const node_sz * const cumulDegs, const node * const neighs);
