...
__global__ void ColoringLuby_k::prune_eligible_clear_is( const uint32_t nnodes, const uint32_t * const coloring_d, bool *const cands_d, bool * const is_d ) {
...
__global__ void ColoringLuby_k::check_conflicts_fast_k( uint32_t nnodes, const node_sz * const cumulDegs, const node * const neighs, bool * const i_i_d ) {
	uint32_t idx = threadIdx.x + blockDim.x * blockIdx.x;
...
__global__ void ColoringLuby_k::update_eligible_fast_k( uint32_t nnodes, const node_sz * const cumulDegs, const node * const neighs, const bool * const i_i_d, bool * const cands_d, bool * const is_d ) {
	uint32_t idx = threadIdx.x + blockDim.x * blockIdx.x;
